#!/usr/bin/env python
"""
TRAIN / Datasets / Real Dataset Ingestion & Preprocessing CLI.
...
4. Caches preprocessed tensors into TRAIN/data/processed/<source>/ for high-performance training.
"""
...
from TRAIN.datasets.adni_loader import ADNIClinicalParser, ADNIMultimodalDataset
from TRAIN.datasets.oasis_loader import OASISClinicalParser, OASISMultimodalDataset
...
def ingest_dataset(
    ...
    print("=" * 75)
    print(f"  TRAIN: REAL DATASET INGESTION & PREPROCESSING ({source.upper()})")
    print("=" * 75)
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="TRAIN: Real Dataset Ingestion & Preprocessing Tool",
...
def main() -> None:
    ...
    if args.raw_dir is None:
        raw_dir = os.path.join(train_root, "data", "raw", src)
    else:
    ...
    if args.output_dir is None:
        output_dir = os.path.join(train_root, "data", "processed", src)
    else:
